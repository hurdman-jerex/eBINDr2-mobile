<?php
$double=ereg("/merge/", $_SERVER['REQUEST_URI']);
define("FONT_SIZE_LOGO",8*(phpversion()<'4.3.0' && $double?2:1));
define("FONT_SIZE_TITLE",14*(phpversion()<'4.3.0' && $double?2:1));
define("FONT_SIZE_AXIS_LABELS",12*(phpversion()<'4.3.0' && $double?2:1));
define("FONT_SIZE_AXIS_MARK_LABELS",8*(phpversion()<'4.3.0' && $double?2:1));
define("FONT_SIZE_LEGEND",8*(phpversion()<'4.3.0' && $double?2:1));
/*
...
function phpdata($a) {
    ...
    if($HZplot->logo) { vhlogo("Created by eBINDr"); }
}
...
class PHPplot {
    ...
    function start($param = "") {
        ...
        ImageFilledRectangle($this->image, 0, 0, $this->size[0],$this->size[1], $background);
//		ImageColorDeallocate($this->image, $background);
    }
    ...
    function show($f = "") {
        ...
        drawaxis();
		if(phpversion()>'4.3.0') imageantialias($HZplot->image, true);
...
class Pplot_axis extends Pplot_base {
    ...
    var $markttf    = false;
//    var $marksize   = 10;
    var $marksize   = FONT_SIZE_AXIS_MARK_LABELS;
    var $markskip   = 1;
    var $markdegree = 0;
...
//    var $labelsize  = 14;
    var $labelsize  = FONT_SIZE_AXIS_LABELS;
    var $labelpos   = "center";
    var $labeldegree= "0";
    ...
    function show() {
        ...
        for($i = 0; $i <= $cnt ; ++$i) {
            ...
            if($this->mark) {
                ...
                $k  = $i ;
                if($this->direction == "x") {
					//$this->markdegree=90; /* Added by Nathan Tanner */
                    if($HZplot->xextend) {
                        $k = $i - 1;
                        if($k <0) { continue ; }
                    }
...
class Pplot_bargraph extends Pplot_base {
    ...
    var $legendfont = 3;
//    var $legendsize = 10;
    var $legendsize = FONT_SIZE_LEGEND;
    var $legendttf  = false;
    var $legendscale= 1;
    var $legendtype = 10;
...
class Pplot_piegraph extends Pplot_bargraph {
    ...
    function show() {
        ...
        for($i = 0; $i < $slices ; ++$i) { 
                $dataset[$i] = round(360 * abs($dataset[$i])/$sum,4);
        }
        # ready to go
        $cx = $this->pos[0]; $cy = $this->pos[1];
        ...
        if(!$this->cubic) {
            ...
            for($i = 0; $i < $slices ; ++$i) {
                ...
                if($this->showmark) {
                    $thisstr = round($dataset[$i] * 10 / 36,1) . '%';
                    $vt = new Pplot_text;
                    $vt->layout(array(
