...
//			console.log(iframe);
		...
		'escape': function( windowEl, iframe, doundo ) {
			...
			if(iframe.window.currentEditr!="") ebindr.button.setExt( iframe.window );
			if( $type(windowEl) == 'element' ) windowEl = ebindr.window.library.Windows.instances.get(windowEl.id);
...
//				if(val>0) document.getElementById("docsimg").src="/css/docs.gif"; else document.getElementById("docsimg").src="/css/docsnone.gif"; break;
	...
	activate: function(selectors) {
		// add the click action/event to the logout button
		$( 'logout' ).addEvent( 'click', function(e) {
			ebindr.modal.confirm( "Are you sure you wish to log out of eBINDr?", function(retValue) {
				if( retValue ) ebindr.logout();
			});
		});
		// Add mouseover to datafields
		$$( 'div.datafields div label' ).addEvents({
			'mouseenter': function(e) {
				$$( this, this.getNext('span') ).addClass( 'hover' );
			},
			'mouseleave': function(e) {
				$$( this, this.getNext('span') ).removeClass( 'hover' );
			}
		});
		// Add mouseover to clicklabel
		$$( 'div.clicklabel' ).addEvents({
			'mouseenter': function(e) {
				$$( this ).addClass( 'hover' );
			},
			'mouseleave': function(e) {
...
//						if( this.id == 'bn' && !ebindr.button.bn ) return true;
		...
		// add the click to the frame itself
		$$( '.frame' ).addEvent( 'click', function(e) {
			this.getElements('u').each( function(el) {
				if( el.getParent().get('tag') == 'h1' ) letter = el.get('text').toLowerCase();
			});
			ebindr.keyboard.frame( letter );
		});
		$$( '.frame h1' ).addEvent( 'click', function(e) {
			this.getElements('u').each( function(el) {
				if( el.getParent().get('tag') == 'h1' ) {
					letter = el.get('text').toLowerCase();
					el.addEvent('click', function(e) {
						// for the 3 main frames member, business, complaint
						switch( letter ) {
							case "m": $('frame_m').contentWindow.location.reload(); break;
							case "c": 
								$('frame_c').contentWindow.location.reload();
								$('frame_x').contentWindow.location.reload();
								$('frame_y').contentWindow.location.reload();
								$('frame_z').contentWindow.location.reload();
								$('frame_customerexperience').contentWindow.location.reload();
								break;
							case "b": $('frame_g').contentWindow.location.reload(); ebindr.data.get('e button info'); 
									//$('frame_history').contentWindow.location.reload(); 
									//$('frame_history').fireEvent('reload','/report/lite button by?noheader&ebindr2=y&&cmd=&newbid=&bid=' + ebindr.current.bid);
									//ebindr.growl('reload bfame test');
								break;
							case "r": 
...
/*		if( button == 'nk' ) {
		...
		//if(button=="masstransfer" && !securitykeys.match("[$]A")) allowed=false;
		if(button=="masstransfercr" && securitykeys.match("bmasstransfer") ) allowed = true;
		...
		if( button != "b?" && !$chk(ebindr.current.bid) || ebindr.current.bid == '(None)' ) {
			...
			if( this.access( button ) ) {
				...
				switch( button ) {
					...
					case "bbararrow":
						this.logbutton( 'bbarrow' );
						$('bizmore-list').setStyles({
							'left': $('bbararrow').getCoordinates().left - 275,
							'top': $('bbararrow').getCoordinates().top + 23
						});
						/*$('bbararrow').addEvent( 'mouseenter', function(e) {*/
							$('bizmore-list').setStyle( 'display', '' ).addEvent( 'mouseleave', function(e) {
								this.setStyle( 'display', 'none' );
							}).getElements('ul')[0].empty();
						/*});*/
						
						var displayed = 0;
						$$('ul.btnbar li').each( function(btn,i) {
						...
						$$( '#bizmore-list li' ).each( function(btn) {
							if( !btn.hasClass('spacer') ) {
...
/*						new ebindr.window.library.Window({
					...
					case "calendarlink":
						...
						ebindr.window.normal( 'calendarlink', {
							id: 'calendar',
					...
					case "bcust-reviews":
						...
						ebindr.window.normal( 'bcust-reviews', {
					...
					case "streaminteractions":
						...
						/*ebindr.window.normal( 'streaminteractions', {
					...
					case "paymethod":
						...
						ebindr.window.normal( 'paymethod', {
					...
					case "iq":
						new ebindr.window.library.Window({
							...
							title: 'Quick View',
							loadMethod: 'iframe',
							contentURL: '/report/lite button iq/?noheader&ebindr2=y&bid=' + ebindr.current.bid,
							width: 800,
							height: window.getSize().y-80
						});
						break;
...
//							ebindr.alert("This record is not an agency");
					...
					case "mn":
						...
						break;
					case "mb":
						ebindr.window.normal( 'lite button mb', {
							id: 'lite button mb',
							//contentURL: '/ebindr/views/video.html?frame=' + ebindr.current.frame.button + '&button=' + ebindr.current.button + '&tab=' + ebindr.current.page,
...
*/
					...
					case "editorderbtn":
						...
						ebindr.window.normal( 'editorderbtn', {
					...
					case "editorderbtnfindr":
						...
						ebindr.modal.confirm( msg, function( ret ) {
							...
							if( ret ) {
								...
								ebindr.window.normal( 'editorderbtnfindr', {
...
/***** if(complaintalert>"") { alert(complaintalert); complaintalert=""; }
					...
					case "customerexperience":
						$$('ul.controls.line2 li').removeClass('selected')
						$(button).addClass('selected');
						$('frame_x').getParent().setStyles({'display':(button=="compliments"?'':'none')});
						$('frame_y').getParent().setStyles({'display':(button=="adreviews"?'':'none')});
						$('frame_z').getParent().setStyles({'display':(button=="customerreviews"?'':'none')});
						$('frame_customerexperience').getParent().setStyles({'display':(button=="customerexperience"?'':'none')});
						$('frame_c').getParent().setStyles({'display':(button=="complaints"?'':'none')});
						$$('div.block.adreviews').setStyles({'display':(button=="adreviews"?'':'none')});
						$$('div.block.complaint').setStyles({'display':(button=="complaints"?'':'none')});
						ebindr.layout.set.autosize();
						break;
					case "cn":
					case "c+":
...
//							return;
//						}
						ebindr.modal.confirm( 'This will allow you to mass-transfer customer reviews on the current business record to another BBB. It will ONLY allow you to transfer reviews that have not already been transferred. Do you wish to proceed?', function( ret ) {
							if( ret ) {
								ebindr.button.editr_edit( "lite button masstransfercr.editr" );
							} else {
								return;
							}
						}, [ 'Yes', 'No' ]);
	...
	fileBrowser: function( type, value ) {
		switch( type ) {
			case "cid":
				ebindr.window.normal( 'filebrowser', {
...
//					var iframe = ebindr.window.iframe( button.replace( / /g, "-" ) + "-" + ebindr.current.bid + '_iframe' ).window;
	...
	//ebindr.growl(ebindr.current.windowEl.id);
			if(ebindr.current.windowEl != null && button != "lite-button-cn" ) {
				var newlocation=ebindr.window.currentlocation( ebindr.current.windowEl.id ); 
