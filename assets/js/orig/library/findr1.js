...
//		$('search-q').addEvent( 'mouseover', ebindr.findr2.findrhistory );
	...
	*/
	more: function() {
		...
		$('more-search').addEvent( 'mouseenter', function(e) {//when the >> button is hovered/clicked
			...
			if(findr2_details.height<480){
				...
				$('more-list').setStyles({
					'width': '600px',
					'left': $('more-search').getCoordinates().left - 310 - 300,
					'top': $('more-search').getCoordinates().top + 10,
					'position': 'absolute',
					'z-index': 10000
				});
	...
	search: function( what, q ) {
		...
		var searchurl = '/report/lite findr ' + what + '/?noheader&ebindr2=y' +
		'&find=' + escape(q) +
