...
/* store report log with parameters */
$__current_query_params_string = implode( ",", $__current_query_params );
$reportr->background->query("replace into ".LOCAL_DB.".reportlog_parameters (mergecode, staff, day, count, params) select ifnull(mergecode,'".$reportr->current_query."'), ifnull(staff,'".$_COOKIE["reportr_username"]."'), ifnull(day,curdate()), ifnull(count+1,1), '".$__current_query_params_string."' from ".LOCAL_DB.".reportlog_parameters where mergecode='".$reportr->current_query."' and staff='".$_COOKIE["reportr_username"]."' and day=curdate() having count(*) in (0,1)");
